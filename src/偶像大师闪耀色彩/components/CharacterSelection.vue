  ...
  <div class="training-selection-screen">
    ...
    <div class="top-decoration">
      <div class="step-indicator">
        <div class="step active">1.アイドル選択</div>
      </div>
      <div class="true-end-badge">
        <i class="fas fa-star"></i>
        <span>True End条件：达成</span>
      </div>
    </div>
          ...
          <div class="attributes-panel">
            ...
            <div class="dimension-circles">
              <div class="circle-item vocal">
                ...
                <div class="stat-value">{{ currentCard.attribute?.stats.vocal || 0 }}</div>
                <div class="percentage">{{ getPercentage(currentCard.attribute?.stats.vocal || 0) }}%</div>
              </div>
              ...
              <div class="circle-item dance">
                ...
                <div class="stat-value">{{ currentCard.attribute?.stats.dance || 0 }}</div>
                <div class="percentage">{{ getPercentage(currentCard.attribute?.stats.dance || 0) }}%</div>
              </div>
              ...
              <div class="circle-item visual">
                ...
                <div class="stat-value">{{ currentCard.attribute?.stats.visual || 0 }}</div>
                <div class="percentage">{{ getPercentage(currentCard.attribute?.stats.visual || 0) }}%</div>
              </div>
            </div>
          ...
          <div class="idol-carousel">
            ...
            <div ref="carouselRef" class="idol-cards-wrapper" @scroll="checkScroll">
              ...
              >
                ...
                <div class="month-badge">強化月間対象</div>
                <div class="idol-stats">
                  <div class="stat-icons">
                    <span class="stat vocal">{{ getGrade(card.attribute?.stats.vocal || 0) }}</span>
                    <span class="stat dance">{{ getGrade(card.attribute?.stats.dance || 0) }}</span>
                    <span class="stat visual">{{ getGrade(card.attribute?.stats.visual || 0) }}</span>
                  </div>
                </div>
              </div>
            </div>
...
import { getCardAttribute } from '../../偶像大师闪耀色彩-gacha/data/card-attributes';
import { getAttributeIcon } from '../../偶像大师闪耀色彩-gacha/data/card-attributes-types';
import type { GachaData } from '../utils/game-data';
import { getGachaData } from '../utils/game-data';
...
// 当前角色的好感度（一个角色的所有卡片共享好感度）
const currentAffection = computed(() => {
  if (!currentCard.value) return 0;
  return affectionMap.value.get(currentCard.value.characterName) || 0;
});
...
const getPercentage = (value: number): string => {
  const percent = ((value / 150) * 100).toFixed(1);
  return percent;
};
